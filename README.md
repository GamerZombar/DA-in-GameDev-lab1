...
- ***Cumulative Reward*** - это общее вознаграждение, которое максимизирует агент. Обычно нужно, чтобы оно увеличивалось, но по некоторым причинам оно может и уменьшаться. Всегда лучше максимизировать вознаграждения в интервале от 1 до -1. В зависимости от сложности задачи  увеличение вознаграждения может не проявиться до тех пор, пока не будет пройдено достаточно большое количество шагов в процессе обучения.
...
- ***Value Loss*** - это средняя потеря функции значения. Она моделирует, насколько хорошо агент прогнозирует значение своего следующего состояния. Должна увеличиваться, пока агент обучается, а затем уменьшаться, когда вознаграждение стабилизируется.

- ***Policy*** - для оценки качества действий в PPO используется концепция политики:
  ...
  - ***Learning Rate*** - соответствует силе обновления градиентного спуска на каждом шагу. Обычно должно линейно уменьшаться.
  ...
  - Также два графика ***Beta*** и ***Epsilon***, про которые было сказано выше.
 
![image](images/13.png)

## Выводы

В ходе лабораторной работы, я узнал как можно интегрировать экономическую систему в проект Unity. Несколько раз обучил ML-Agent и узнал как параметры конфигурации нейросети влияют на обучение модели. Научился выводить графики в _TensorBoard_ и анализировать их.
